...
def тест_интероперабельности(режим: str):
    ...
    try:
        ...
        try:
            ...
            # Извлекаем IV и data для OpenSSL
            iv_из_файла, data_без_iv = ОбработчикIV.извлечь_iv_из_данных(data_с_iv)
            
            # Сохраняем data без IV для OpenSSL
            data_для_openssl = оригинальный_файл + '.openssl_in'
        ...
        try:
            ...
            with open(cryptocore_расшифрованный, 'wb') as f:
                f.write(расшифрованные_data)
            
            # Сравниваем
            if расшифрованные_data == тестовый_текст:

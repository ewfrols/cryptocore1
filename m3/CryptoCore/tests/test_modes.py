...
def тест_режима(название_режима: str, требует_дополнения: bool):
    ...
    try:
        ...
        for i, данные in enumerate(тестовые_данные, 1):
            ...
            if название_режима == 'ecb' and len(данные) == 0:
...
def тест_файлового_ввода_вывода():
    ...
    ключ = "000102030405060708090a0b0c0d0e0f"
    тестовый_текст = b"Тестовые данные для проверки работы с файлами и IV."
        ...
        try:
            ...
            try:
                ...
                with open(зашифрованный_файл, 'rb') as f:
                    прочитанные_данные = f.read()
                
                # Извлекаем IV и данные
                iv_из_файла, данные_без_iv = ОбработчикIV.извлечь_iv_из_данных(прочитанные_данные)
